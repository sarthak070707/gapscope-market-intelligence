...
export function classifyError(
  ...
  if (error instanceof Error) {
    ...
    if (alreadyClassified) {
      ...
      if (existingPrefix === 'RATE_LIMIT' || msg.includes('429') || msg.includes('rate limit')) { category = 'rate_limit'; statusCode = 429; }
      else if (existingPrefix === 'TIMEOUT' || msg.includes('timeout') || msg.includes('timed out')) category = 'timeout';
      else if (existingPrefix === 'AUTH' || msg.includes('unauthorized') || msg.includes('forbidden')) { category = 'auth'; statusCode = msg.includes('forbidden') ? 403 : 401; }
      ...
      else if (existingPrefix === 'NOT_FOUND') { category = 'api'; statusCode = 404; }
      else if (existingPrefix === 'BAD_GATEWAY') { category = 'api'; statusCode = 502; }
      else if (existingPrefix === 'SERVER_ERROR') { category = 'api'; statusCode = 500; }
      else if (existingPrefix === 'SERVICE_UNAVAILABLE') { category = 'api'; statusCode = 503; }
      else if (existingPrefix === 'NETWORK') category = 'api';
      ...
      return {
        ...
        possibleReason: `Error at stage ${extractedStage || existingPrefix || 'unknown'}: ${error.message.substring(0, 300)}. Check Debug Info for full stack trace.`,
    ...
    if (error instanceof TypeError || error instanceof ReferenceError) {
      ...
      return {
        ...
        possibleReason: isDbRelated
          ? `Database error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Try running a scan first.`
          : `Parse error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. This may be caused by a malformed AI response. Retrying usually resolves this.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (error instanceof RangeError) {
      return {
        ...
        possibleReason: `Range error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. This may be caused by unexpected data from the AI model.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (error instanceof SyntaxError) {
      return {
        ...
        possibleReason: `JSON parse error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. This may be caused by malformed request data or an invalid AI response.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('429') || msg.includes('rate limit') || msg.includes('too many requests') || msg.includes('throttl')) {
      return {
        ...
        detail: error.message,
        possibleReason: `Rate limited at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Wait 30-60 seconds before retrying.`,
        retryable: true,
        timestamp,
        endpoint,
        statusCode: 429,
    ...
    if (msg.includes('unauthorized') || msg.includes('forbidden') || 
      ...
      return {
        ...
        detail: error.message,
        possibleReason: `Auth error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Check ZAI_API_KEY environment variable.`,
        retryable: false,
        timestamp,
        endpoint,
        statusCode,
    ...
    if (msg.includes('failed to parse ai response') || 
      ...
      return {
        ...
        detail: error.message,
        possibleReason: `AI parse error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Retrying usually resolves this.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('empty response') || msg.includes('ai returned an empty')) {
      return {
        ...
        message: `[AI_RESPONSE] ${error.message}`,
        detail: error.message,
        possibleReason: `AI empty response at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. The model may be overloaded. Try again.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('ai chat completion failed') || msg.includes('ai generation') || msg.includes('ai server error')) {
      return {
        ...
        detail: error.message,
        possibleReason: `AI API error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Usually transient — retry after a short wait.`,
        retryable: true,
        timestamp,
        endpoint,
        statusCode: 500,
    ...
    if (msg.includes('web search failed') || msg.includes('page reading failed')) {
      return {
        ...
        message: `[API] ${error.message}`,
        detail: error.message,
        possibleReason: `Web API error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. The search/page service may be temporarily unavailable.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('fetch') || msg.includes('network') || msg.includes('econnrefused') || msg.includes('enetunreach')) {
      return {
        ...
        detail: error.message,
        possibleReason: `Network error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Check connectivity.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('prisma') || msg.includes('sqlite') || msg.includes('database') || 
      ...
      return {
        ...
        detail: error.message,
        possibleReason: `Database error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Try running a scan first.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('404') || msg.includes('not found')) {
      return {
        ...
        detail: error.message,
        possibleReason: `Not found at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Try scanning first.`,
        retryable: false,
        timestamp,
        endpoint,
        statusCode: 404,
    ...
    if (msg.includes('500') || msg.includes('internal server')) {
      return {
        ...
        detail: error.message,
        possibleReason: `Server error (500) at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Usually transient — retry.`,
        retryable: true,
        timestamp,
        endpoint,
        statusCode: 500,
    ...
    if (msg.includes('502') || msg.includes('bad gateway')) {
      return {
        ...
        detail: error.message,
        possibleReason: `Bad Gateway (502) at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. The gateway/proxy could not reach the backend. This usually means the backend server was too slow or temporarily unavailable. Wait a moment and try again.`,
        retryable: true,
        timestamp,
        endpoint,
        statusCode: 502,
    ...
    if (msg.includes('503') || msg.includes('service unavailable') || msg.includes('overloaded')) {
      return {
        ...
        detail: error.message,
        possibleReason: `Service unavailable at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Try again later.`,
        retryable: true,
        timestamp,
        endpoint,
        statusCode: 503,
    ...
    if (msg.includes('empty') || msg.includes('no result') || msg.includes('no data') || msg.includes('nothing found')) {
      return {
        ...
        detail: error.message,
        possibleReason: `No results at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Try a different category or time period.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('invalid') || msg.includes('malformed') || msg.includes('serialize') ||
      ...
      return {
        ...
        detail: error.message,
        possibleReason: `Validation error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Check input parameters.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('missing category') || msg.includes('category is required')) {
      return {
        ...
        message: `[VALIDATION] ${error.message}`,
        detail: error.message,
        possibleReason: `Missing category at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Select a category and try again.`,
        retryable: false,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('abort') || msg.includes('cancel')) {
      return {
        ...
        message: `[TIMEOUT] ${error.message}`,
        detail: error.message,
        possibleReason: `Request aborted at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. May be due to timeout.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('bad request')) {
      return {
        ...
        message: `[VALIDATION] ${error.message}`,
        detail: error.message,
        possibleReason: `Bad request at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Check input parameters.`,
        retryable: false,
        timestamp,
        endpoint,
        statusCode: 400,
    ...
    if (msg.includes('constraint') || msg.includes('foreign key') || msg.includes('unique') ||
      ...
      return {
        ...
        detail: error.message,
        possibleReason: `DB constraint error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Try running a fresh scan.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('migration') || msg.includes('schema') || msg.includes('column') ||
      ...
      return {
        ...
        detail: error.message,
        possibleReason: `Schema error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Run "bun run db:push" to update.`,
        retryable: false,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('timed out') || msg.includes('timeout') || msg.includes('deadline exceeded') ||
      ...
      return {
        ...
        message: `[TIMEOUT] ${error.message}`,
        detail: error.message,
        possibleReason: `Timeout at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Try with a narrower scope.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    if (msg.includes('slow down') || msg.includes('wait before') || msg.includes('try again later') ||
      ...
      return {
        ...
        message: `[RATE_LIMIT] ${error.message}`,
        detail: error.message,
        possibleReason: `Rate limit at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Wait 60 seconds before retrying.`,
        retryable: true,
        timestamp,
        endpoint,
        statusCode: 429,
    ...
    if (msg.includes('completion') || msg.includes('generation') || msg.includes('model') ||
      ...
      return {
        ...
        message: `[AI_RESPONSE] ${error.message}`,
        detail: error.message,
        possibleReason: `AI error at stage ${extractedStage || 'unknown'}: ${error.message.substring(0, 200)}. Input may be too long or triggered content policy.`,
        retryable: true,
        timestamp,
        endpoint,
        ...errorOrigin,
    ...
    return {
      ...
      detail: `[${error.constructor.name}] ${error.message}`,
      possibleReason: `Error at stage ${extractedStage || 'unknown'} (${error.constructor.name}): ${error.message.substring(0, 300)}. Check Debug Info for full stack trace.`,
      retryable: true,
      timestamp,
      endpoint,
      ...errorOrigin,
  ...
  return {
    ...
    possibleReason: `Non-Error at stage ${nonErrorStage || 'unknown'}: ${errorStr.substring(0, 200)}. Check server logs.`,
    retryable: true,
    timestamp,
    endpoint,
    stage: nonErrorStage,
